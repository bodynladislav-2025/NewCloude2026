...
export default function AddMatch({ players, matches, setMatches, editingMatch, setEditingMatch, setActiveTab }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (form.player1Id === form.player2Id) {
      setError('Hráč 1 a Hráč 2 musí být různí.');
      return;
    }
    const s1 = parseInt(form.score1, 10);
    const s2 = parseInt(form.score2, 10);
    if (isNaN(s1) || isNaN(s2) || s1 < 0 || s2 < 0) {
      setError('Zadejte platné skóre (nezáporná čísla).');
      return;
    }
    if (!form.date) {
      setError('Zadejte datum zápasu.');
      return;
    }
      ...
      setMatches((prev) =>
        prev.map((m) =>
          m.id === editingMatch.id
            ? { ...m, date: form.date, player1Id: form.player1Id, player2Id: form.player2Id, score1: s1, score2: s2 }
            : m
        )
      );
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-4">
            ...
            <div className="flex gap-3 items-end">
              <div className="flex-1">
                ...
                <select
                  ...
                  onChange={(e) => handleChange('player1Id', e.target.value)}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-8 h-8 rounded-full bg-yellow-400 flex items-center justify-center text-xs font-bold text-yellow-900 shadow-sm">
                VS
              </div>
              <div className="flex-1 h-px bg-gray-200" />
            </div>
            ...
            <div className="flex gap-3 items-end">
              <div className="flex-1">
                ...
                <select
                  ...
                  onChange={(e) => handleChange('player2Id', e.target.value)}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-green-400 focus:ring-2 focus:ring-green-100 outline-none text-gray-700 bg-white transition-all"
                >
                  <option value="">— Vyberte hráče —</option>
                  {availableP2.map((p) => (
          ...
          {/* Error */}
          {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm fade-in">
              ⚠️ {error}
            </div>
          )}
          ...
          {/* Success */}
          {success && (
            <div className="bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm fade-in">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              className="flex-1 px-4 py-3 rounded-xl bg-green-500 text-white font-semibold hover:bg-green-600 transition-colors shadow-md"
            >
              {editingMatch ? 'Uložit změny' : '+ Přidat zápas'}
            </button>
          </div>
        </form>
      </div>
